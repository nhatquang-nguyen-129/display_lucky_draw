...
Text) khai báo domain riêng cho từng caller, Background/Image truyền đủ cả 4 giá trị. Dim/Blur có thêm
field **Amount** (Dim % / Blur px, chỉ hiện khi Appearance đang chọn đúng 1 trong 2 giá trị đó) —
Text không bao giờ đụng tới field này. Xem doc-comment `DrawCycleConfig`/`DrawPhaseAction` trong
...
| `ScoreboardPanel.tsx` | Scoreboard — "Columns" là bản multi-select của nhánh 1 (liệt kê MỌI cột thật, không lọc Data Type) nhưng KHÔNG có tiêu chí phụ nào (bảng hiện được bất kỳ cột nào), nên không có khái niệm Eligible/not eligible ở đây — mọi cột luôn chọn được. Giao diện là 1 dropdown chọn nhiều (`ColumnsDropdown`, cuối Basic options): nút trông như 1 select thường hiện tên các cột đang chọn, bấm mở danh sách có dấu tick bên cạnh cột đang chọn, bấm 1 dòng để bật/tắt (danh sách vẫn mở để chọn tiếp), bấm ra ngoài hoặc Esc để đóng. Title trên Name Bar luôn căn giữa (`TableTemplate.tsx`) |
