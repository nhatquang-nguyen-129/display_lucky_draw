...
// Vẽ 2 effect thuộc nhóm "overlay-category" (glow/sweep — thêm 1 LỚP PHỦ RIÊNG trên ảnh, xem
// doc-comment PrizeStageEffect trong types.ts). Effect thuộc nhóm "transform-category"
...
export default function PrizeEffectOverlay({
  ...
  if (effect === "glow") {
    ...
    if (mode === "persistent") {
      // `mixBlendMode: "screen"` — ánh sáng CỘNG DỒN vào ảnh bên dưới thay vì đè phẳng (giống hệt
      // Sweep bên dưới) — thiếu nó thì alpha cao cỡ nào cũng không "rực" được.
    ...
    // Oneshot (When Won) — cùng công thức lõi-cố-định/lan-toả-theo-size, nhưng chạy qua CSS keyframes
    // (prize-fx-glow-pulse trong landingEffects.css) nên truyền `size` vào bằng custom property thay
    // vì tính filter string trực tiếp ở đây — `currentColor` (từ `color` ở style cha) không tách được
    // alpha riêng theo size như bản persistent, bù lại bằng cách STACK nhiều lớp full-opacity cùng
    // `mixBlendMode: "screen"` (mỗi lớp chồng thêm càng cộng dồn sáng hơn) để size lớn hơn vẫn rực hơn
    // thấy rõ trong đúng 1 nhịp chớp ngắn.
    return (
      <div
