...
// trong types.ts. "sweep"/"spotlight" không có trong map này (không dùng field size — spotlight dùng
// `delayMs` riêng, xem SPOTLIGHT DELAY bên dưới). "scaleUp" VÀ "lift" CŨNG không còn ở đây nữa — mức
// độ giờ kéo-thả trực tiếp trên canvas (điểm "Direction", xem ScaleAnchorOverlay.tsx), không còn ô
// nhập số riêng.
...
function GroupEffectFields({
  ...
  // liệt kê loại trừ ("khác none và khác sweep") — bản loại trừ từng gây bug thật: landing cũ lỡ lưu 1
  // effect KHÔNG có field size (vd "sweep"/"spotlight") vẫn "khác none và khác sweep" nên field Amount
  // vẫn hiện ra dù dropdown hiển thị "None" (effect không khớp option nào, trình duyệt tự hiện option
  // ĐẦU TIÊN) — danh sách trắng tự động AN TOÀN với mọi effect lạ/cũ, không cần liệt kê riêng từng
  // trường hợp.
  const showSize = SIZE_LABEL[value.effect] !== undefined;
  // Delay (ms) — CHỈ spotlight dùng (xem doc-comment PrizeGroupEffect.delayMs trong types.ts).
  const showDelay = value.effect === "spotlight";
          ...
          {showDelay && (
            <div>
              ...
              <input
                ...
                onChange={(e) => onChange({ delayMs: Math.max(0, Number(e.target.value)) })}
              />
            </div>
          )}
        </div>
...
// Panel con DÙNG CHUNG cho cả 4 mục Hover/Select/Won/Out of Stock (LiveImagePanel.tsx — `title` truyền
// vào KHÔNG còn tiền tố "When " nữa, bỏ cho gọn) — GỘP 3 nhóm ĐỘC LẬP (Focus/Highlight/Motion, xem
// PRIZE_EFFECT_GROUPS trong types.ts) trong CÙNG 1 <details>: mỗi nhóm tự có dropdown + field phụ
// riêng (color/size/direction/delay theo đúng effect nhóm đó đang chọn), và có thể BẬT ĐỒNG THỜI (vd
// vừa Focus vừa Highlight) — chỉ riêng TRONG 1 nhóm mới giới hạn ĐÚNG 1 effect. Cả 4 mục giờ dùng
// CHUNG Y HỆT 1 khuôn (không còn mục nào có nội dung riêng qua `children` như trước — Spotlight/Dim đã
// gộp thành 2 lựa chọn bình thường của nhóm Highlight, xem doc-comment PrizeEffectName trong
// types.ts). Thứ tự trong <details>: **Appearance** (đặt LÊN ĐẦU — dropdown ĐƠN GIẢN nhất, không có
// field phụ nào, hợp lý để lướt qua trước) → Focus/Highlight/Motion. Bọc trong <details> — mặc định
// MỞ nếu BẤT KỲ nhóm nào đang khác "none" (không giấu cấu hình đã set), sau đó hoàn toàn do người dùng
// tự đóng/mở (không ép lại theo re-render — cùng kỹ thuật `useState(() => ...)` đã dùng cho "Interact
// with Wheel" ở BackgroundPanel.tsx).
