...
- **Landing component chủ yếu thuộc 1 trong 2 nhóm — Signal Emitter hoặc Signal Receiver**: đây là nguyên tắc kiến trúc cố định cho Landing Builder (`src/components/landing/`), áp dụng cho MỌI component hiện có lẫn tương lai. Emitter (Button hôm nay; sau này QR Scanner/NFC Reader/Keyboard Shortcut...) là component người vận hành thao tác trực tiếp trong Present Mode — CHỈ phát Event (vd `Button.Click`), không tự chạy business logic/animation nào, không biết Draw/Confirm/Fireworks/Lucky Wheel là gì. Receiver (Lucky Wheel, Fireworks, Stage Light, Dim Background, Link Opener, Draw, Confirm Winner hôm nay; Countdown, Winner Banner, Prize Image, Scoreboard, Video, Music... sau này) CHỈ nhận Command từ Trigger Graph để tự thực thi animation/logic của chính nó (vd `Wheel.StartSpin`) — người vận hành KHÔNG thao tác trực tiếp lên Receiver ở Present Mode. **Ngoại lệ hẹp có kiểm soát**: 1 Receiver có animation/thao tác hoàn thành sau khoảng thời gian KHÔNG cố định (tuỳ cấu hình hoặc tuỳ 1 lời gọi IPC bất đồng bộ, vd Lucky Wheel hoặc Draw) được phép có thêm ĐÚNG 1 emit báo "xong việc nó vừa được giao" (Lucky Wheel: `Wheel.SpinCompleted`, bắn qua `sequence.fireClick()` ngay tại điểm animation thật kết thúc — xem `WheelTemplate.tsx`/`DigitRollerTemplate.tsx`; Draw: `Draw.Picked`, bắn ngay khi `sequence.pick()` thật sự thành công — xem `DrawView.tsx`) để nối chuỗi hành động (vd Wheel quay xong → Fireworks bắn, hoặc Draw chọn xong → Wheel bắt đầu quay) chính xác, không phải đoán 1 delay cố định rồi hy vọng khớp. Đây KHÔNG phải "component tự do vừa Emitter vừa Receiver" — chỉ áp dụng cho đúng 1 tín hiệu hoàn-thành-việc-của-chính-nó, không phải danh sách emit tuỳ ý. **Ngoại lệ hẹp thứ 2 — "Gateable Emitter"**: 1 Emitter (hôm nay là Button) được phép `listensFor` thêm ĐÚNG 2 Command dùng chung theo khuôn `<Type>.Enable`/`<Type>.Disable` (Button: `Button.Enable`/`Button.Disable`) để 1 Signal bất kỳ trên Graph khoá/mở nó lúc runtime (vd `Wheel.SpinCompleted → Confirm.Enable` khiến nút Confirm chỉ bấm được sau khi quay xong, xem `docs/graph.md` mục 1 và `ButtonView.tsx`) — kèm 1 field `startEnabled` trên chính component quy định trạng thái trước khi Command đầu tiên tới. Component vẫn KHÔNG đọc thêm bất kỳ state nghiệp vụ nào (không biết Draw/Wheel là gì) — chỉ nhớ đúng 1 boolean do 2 Command này set, giữ nguyên tinh thần "vỏ rỗng, chỉ phản ứng theo dây đã nối". Trigger Graph (`triggerGraph/`) là tầng trung gian DUY NHẤT nối Emitter → Receiver (Event → Command), không bao giờ biết Receiver thực thi cụ thể ra sao. Tên tín hiệu luôn theo quy ước `Component.Action` (vd `Wheel.StartSpin`, `Fireworks.Play`) — khai báo tập trung ở `COMPONENT_SIGNALS` trong `componentRegistry.ts`. Thêm loại component mới PHẢI xác định rõ Emitter/Receiver/cả-hai-với-ngoại-lệ-hẹp trước khi code (xem checklist ở đầu `src/lib/landing/types.ts`).
